class Api::BaseTimelineController < ApiController
  ...
  def editItemBase
    ...
    begin
      ...
      if i.save
        my_logger.info('USER '+session[:user]['user']['id'].to_s+' EDITED item base timeline => '+i.id.to_s)
        status[:success] = true
      else
  ...
  def deleteItemBase
    ...
    begin
      ...
      if item.delete
        my_logger.info('USER '+session[:user]['user']['id'].to_s+' DELETED item base timeline => '+item.id.to_s)
        status[:success] = true
      else
  ...
  def setJson
    ...
    begin
      ...
      if params[:json]
        ...
        if base.save
          my_logger.info('USER '+session[:user]['user']['id'].to_s+' REFRESHED json base timeline => '+base.id.to_s)
          response[:success] = true
        else
