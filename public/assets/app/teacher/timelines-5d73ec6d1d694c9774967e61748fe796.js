$(function(){
    $('.go').on('click', function(){
        window.location.href = "/professor/timelines/"+$('[name=year]').val()+"/"+$('[name=half]:checked').val()+"/"+$('[name=tcc]:checked').val();
    });
    var base_element = '<a href="/professor/timelines/{{year-half}}/{{tcc}}">{{text}}</a>'
    var date = new Date();
...
function set_timeline(timeline_id){
  ...
  for(i in items){
    if(items[i].link != '#'){
      items[i].link = items[i].link.replace('academico', 'professor/timeline/'+timeline_id);
    }else{
      items[i].link = "http://localhost:3000/professor/timeline/"+timeline_id+"/item/"+items[i].id;
    }
  }
